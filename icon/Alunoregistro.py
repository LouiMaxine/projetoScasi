...
class CadastroAluno (QDialog):#Terceira etapa -Criação do Formulário do Aluno
    ...
    def AddAluno(self):
        ...
        try:
            ...
            QMessageBox.information(QMessageBox(),'Cadastro','Cadastro do aluno realizado com sucesso!')
            self.close()

        except Exception:
            QMessageBox.warning(QMessageBox(),'br07louise@hotmail.com','Não foi possível realizar o cadastro, verifique se os campos foram preenchidos corretamente.')
...
class PesquisarAluno(QDialog):#Quarta etapa
    ...
    def pesquisandoAluno(self):
        ...
        try:
            ...
            QMessageBox.information(QMessageBox(),'Pesquisa realizada com sucesso!',result_pesq)
            conn.commit()
            cur.close()
            conn.close()

        except Exception:
            QMessageBox.warning(QMessageBox(),'br07louise@hotmail.com','Não foi possível realizar a pesquisa, verifique se o número da inscrição está correto')
...
class Deletar(QDialog):#Quinta etapa
    def __init__(self,*args,**kwargs):
        ...
        self.btn_del_aln.clicked.connect(self.deletando)#Conectando a função a baixo
        ...
        self.del_somente_num = QIntValidator()#Só aceita números, e não dixa escrever por extenso com letras
    ...
    def deletando(self):
        ...
        try:
            ...
            QMessageBox.information(QMessageBox(),'br07louise@hotmail.com', 'Inscrição deletada com sucesso!')
            self.close()
        except Exception:
            QMessageBox.warning(QMessageBox(),'br07louise@hotmail.com','Não foi possível deletar, verifique se o número da inscrição está correto')
...
class SobreAjuda (QDialog):#Segunda etapa
    ...
    Campus Valença''')
        ...
        layout.addWidget(QLabel("v2.O"))#Mais um label
...
class Login(QDialog):
    def __init__(self, parent = None):
        ...
        self.setFixedWidth(405)#fixa largura da janela
        self.setFixedHeight(300)
        ...
        layout.addWidget(self.lbl_nome_usu,2,1)#daqui
        font4 = self.lbl_nome_usu.font()#Recebendo a fonte do título
        font4.setPointSize(15)
        self.lbl_nome_usu.setFont(font4)
        layout.addWidget(self.lbl_senha,3,1)#até aqui layout do label
        font5 = self.lbl_senha.font()#Recebendo a fonte do título
        ...
        font = self.lbl_boas.font()#Recebendo a fonte do título
        font.setPointSize(15)
        self.lbl_boas.setFont(font)


        layout.addWidget(self.nome_txt,2,2)
        font2 = self.nome_txt.font()#Recebendo a fonte do título
        font2.setPointSize(12)
        self.nome_txt.setFont(font2)
        layout.addWidget(self.senha_txt,3,2)
        font3 = self.senha_txt.font()#Recebendo a fonte do título
        font3.setPointSize(12)
        self.senha_txt.setFont(font3)
        layout.addWidget(self.btn_Login,7, 1, 1, 2)#localizar o botão
        font6 = self.btn_Login.font()#Recebendo a fonte do título
        font6.setBold(True)
    ...
    def gerar_login(self):
        if(self.nome_txt.text()=='admin' and self.senha_txt.text()== 'admin'):#Se o texto for igual a admin senha e usuário, ele vai aceitar
            self.accept()
        else:
...
class MainWindow (QMainWindow):#Primeira etapa
    def __init__(self,*args,**kwargs):
        ...
        self.setWindowIcon (QIcon('icon/g2.png'))#até aqui é possível abrir a janela com o ícone
        ...
        def criar_tabelas():
            try:
                ...
                sql = "CREATE TABLE estudantes (id INT AUTO_INCREMENT PRIMARY KEY, nome VARCHAR(255) NOT NULL, nomecurso VARCHAR(100),semestre VARCHAR(25),tel BIGINT(14) NOT NULL,endereco VARCHAR(255) NOT NULL)DEFAULT CHARACTER SET utf8"
                cursor.execute(sql)
                db.commit()
                db.close
                cursor.close()
        ...
        self.setWindowTitle ("CADASTRO DE ALUNOS DO SUPERIOR IFBA - Campus Valença:")
        ...
        sobre_desenv = QAction(QIcon("icon/i1.png"),"Desenvolvedor",self)#Para crir o menu de ajuda
...
if (QDialog.Accepted == True):
    ...
    sys.exit(app.exec_())#até aq, faz parte da config que faz a tela abrir só com o ícone
